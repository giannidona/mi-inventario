...
export function InventoryApp({ profileUsername }: InventoryAppProps) {
  ...
  useEffect(() => {
    ...
    async function loadProfileInventory() {
      ...
      setProducts([])
      try {
        const profile = await getProfileByUsername(supabase, normalizedSlug)
        if (cancelled) return
        setPublicProfile(profile)
        if (!profile) {
          return
        }
        const items = await fetchInventoryByUsername(supabase, normalizedSlug)
        if (!cancelled) setProducts(items)
      } catch (e) {
        console.error(e)
        if (!cancelled) {
          toast.error(
            "No se pudo cargar el inventario. Ejecuta la migración de perfiles en Supabase (SQL 20260205400000)."
          )
        }
      } finally {
    ...
    void loadProfileInventory()
    return () => {
      cancelled = true
    }
  }, [supabase, normalizedSlug])
  ...
  const isOwnProfile = Boolean(
    contributor &&
      session?.user?.id &&
      publicProfile?.id &&
      session.user.id === publicProfile.id
  )

  useEffect(() => {
    if (!session?.user?.id || !contributor) {
      setMyUsername(null)
      return
    }
    let cancelled = false
  ...
  }, [session?.user?.id, contributor, supabase])
  ...
  const openAddItemSheet = useCallback(() => {
    ...
    toast.message("Publicás en tu propio perfil", {
      description: "Abrí tu enlace personal para añadir ítems.",
    })
  }, [contributor, isOwnProfile])
  ...
  useEffect(() => {
    if (!isOwnProfile && isAddSheetOpen) {
  ...
  const handleAddProduct = async () => {
    ...
    if (!isOwnProfile || !uid) {
      toast.error("Solo podés guardar ítems en tu propio perfil.")
      return
    }
    ...
    <div className="flex min-h-dvh flex-col bg-[#FFFFFF] md:flex-row">
      ...
      <aside className="hidden md:flex md:w-52 md:shrink-0 md:flex-col md:border-r md:border-[#F0F0F0] md:bg-[#FFFFFF] md:py-6 md:px-3">
        <div className="px-3 pb-6">
          <p className="text-xs font-semibold tracking-[0.2em] text-[#000000]">Mi inventario</p>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="sticky top-0 z-40 flex items-center justify-between border-b border-transparent bg-[#FFFFFF] px-4 py-4 md:border-[#F0F0F0] md:px-8 lg:px-10">
          ...
          <h1 className="absolute left-1/2 flex -translate-x-1/2 flex-col items-center md:hidden">
            <span className="text-xs font-semibold tracking-[0.2em] text-[#000000]">
              Mi inventario
            </span>
            <span className="text-[10px] font-medium text-[#888888]">@{normalizedSlug}</span>
          </h1>
          <div className="hidden md:flex-1 md:block" aria-hidden />
          <div className="ml-auto flex items-center gap-2 md:ml-0">
            {contributor && !isOwnProfile && myUsername && (
              <button
                type="button"
                onClick={() => router.push(`/${myUsername}`)}
                className="hidden rounded-md px-2 py-1.5 text-xs font-medium text-[#000000] hover:bg-[#F5F5F5] sm:inline"
              >
                Mi @{myUsername}
              </button>
            )}
            ...
            <div className="mx-auto w-full max-w-[1600px] px-4 sm:px-6 md:px-8 lg:px-10">
              {inventoryLoading ? (
                <div className="flex min-h-[45vh] flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-xs text-[#888888]">Cargando…</p>
                </div>
              ) : !publicProfile ? (
                <div className="flex min-h-[45vh] flex-col items-center justify-center gap-3 px-4 text-center">
                  <p className="text-sm font-medium text-[#000000]">No existe @{normalizedSlug}</p>
                  <p className="max-w-sm text-xs text-[#888888]">
                    Comprobá el enlace o pedile a la persona su usuario correcto.
                  </p>
                  <Button
                    type="button"
                    variant="outline"
                ...
                <div className="flex min-h-[45vh] flex-col items-center justify-center gap-2 px-4 text-center">
                  ...
                  <p className="max-w-sm text-xs text-[#888888]">
                    {isOwnProfile
                      ? "Añadí una foto para crear tu primer ítem."
                      : "Este usuario aún no publicó nada en su inventario."}
                  </p>
                  <Button
                    type="button"
                    className="mt-2 bg-[#000000] text-[#FFFFFF] hover:bg-[#333333]"
                    onClick={() => {
                      if (isOwnProfile || !contributor) {
                        openAddItemSheet()
                        return
                      }
                      if (myUsername) router.push(`/${myUsername}`)
                    }}
                  >
                    {isOwnProfile
                      ? "Añadir ítem"
        ...
        {activeTab === "profile" && (
          <div className="mx-auto w-full max-w-2xl px-4 py-4 sm:px-6 md:px-8 lg:px-10">
            <div className="mb-8 flex flex-col items-center">
              ...
              {contributor ? (
                <p className="mt-1 max-w-xs text-center text-xs text-[#888888]">
                  ...
                  {publicProfile ? (
                    <span className="mt-1 block text-[#000000]">@{publicProfile.username}</span>
                  ) : null}
                </p>
              ) : (
                <p className="mt-2 max-w-xs text-center text-xs text-[#888888]">
                  {publicProfile
                    ? `Inventario de @${publicProfile.username}. Entrá con tu cuenta para crear el tuyo.`
            ...
            {publicProfile && (
              <div className="mb-6 rounded-md border border-[#F0F0F0] bg-[#FAFAFA] p-3">
                <p className="text-[10px] font-medium uppercase tracking-wide text-[#888888]">
                  Enlace público
                </p>
                <p className="mt-1 break-all text-xs text-[#000000]">
                  {typeof window !== "undefined"
                    ? `${window.location.origin}/${publicProfile.username}`
                    : `…/${publicProfile.username}`}
                </p>
                <button
                  type="button"
                  className="mt-2 text-xs font-medium text-[#000000] underline underline-offset-2"
                  onClick={() => {
                    const url = `${typeof window !== "undefined" ? window.location.origin : ""}/${publicProfile.username}`
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-3 border-b border-[#F0F0F0]">
                <span className="text-sm text-[#000000]">Ítems en este perfil</span>
